...
function kleo_base_dynamic_variables( $variables ) {
    ...
    if ( sq_option( 'header_height', '' ) ) {
        $variables['header-height'] = kleo_set_default_unit( sq_option('header_height') );
    }
    if ( sq_option('container_padding_left') ) {
        $variables['container-padding-left'] = kleo_set_default_unit(sq_option('container_padding_left') );
    }
    if ( sq_option('container_padding_right') ) {
        $variables['container-padding-right'] = kleo_set_default_unit( sq_option( 'container_padding_right' ) );
    }
    if ( sq_option('side_padding_left') ) {
        $variables['sidemenu-padding-left'] = kleo_set_default_unit( sq_option('side_padding_left'));
    }
    if ( sq_option('side_padding_right') ) {
        $variables['sidemenu-padding-right'] = kleo_set_default_unit( sq_option('side_padding_right'));
    }
    if ( sq_option('logo_padding_left') ) {
        $variables['logo-padding-left'] = kleo_set_default_unit( sq_option('logo_padding_left'));
    }


    if ( sq_option('sidemenu_width') ) {
        $variables['sidemenu-width'] = kleo_set_default_unit( sq_option('sidemenu_width') );
    }

    if ( sq_option('sidemenu_col_width') ) {
        $variables['sidemenu-collapsed-width'] = kleo_set_default_unit( sq_option('sidemenu_col_width') );
    }

    if ( sq_option('sidemenu_header_bg') ) {
        $variables['sidemenu-header-background-color'] = sq_option('sidemenu_header_bg');
    }
    
    if ( sq_option('sidemenu_font_size') ) {
        $variables['menu-font-size'] = kleo_set_default_unit( sq_option('sidemenu_font_size') );
    }

    if ( sq_option('sidemenu_submenu_font_size') ) {
        $variables['submenu-font-size'] = kleo_set_default_unit( sq_option('sidemenu_submenu_font_size') );
    }

	if ( sq_option('blog_full_bg', '') != '' ) {
        $variables['blog-full-hero-background'] = sq_option('blog_full_bg', '');
    }

    if ( sq_option('sidemenu_menu_active_bg', '') != '' ) {
        $variables['sidemenu-menu-background'] = sq_option('sidemenu_menu_active_bg', '');
    }
    if ( sq_option('sidemenu_submenu_active_bg', '') != '' ) {
        $variables['sidemenu-active-submenu-background'] = sq_option('sidemenu_submenu_active_bg', '');
    }

    //sidemenu_menu_active_bg
	//sidemenu_submenu_active_bg
...
function kleo_base_settings( $kleo )
{
    ...
    $kleo['set'][] = array(
        'id' => 'sidemenu_font_size',
        'type' => 'text',
        'title' => 'Menu Font size',
        'default' => '',
        'section' => 'kleo_section_side_styling',
        'description' => 'Set font size in px,em. Default 14px',
        'customizer' => true,
        'transport' => 'refresh'
    );

        $kleo['set'][] = array(
        'id' => 'sidemenu_submenu_font_size',
        'type' => 'text',
    ...
    $kleo['set'][] = array(
        'id' => 'sidemenu_header_bg',
        'type' => 'color',
        'title' => 'Sidemenu Logo Background',
        'default' => '',
        'section' => 'kleo_section_side_styling',
        'customizer' => true,
        'transport' => 'refresh'
    ...
    $kleo['set'][] = array(
        'id' => 'sidemenu_menu_active_bg',
        'type' => 'color',
        'title' => 'Sidemenu Active Menu Background',
        'default' => '',
        'section' => 'kleo_section_side_styling',
        'customizer' => true,
        'transport' => 'refresh'
    ...
    $kleo['set'][] = array(
        'id' => 'sidemenu_submenu_active_bg',
        'type' => 'color',
        'title' => 'Sidemenu Active Sub-Menu Background',
        'default' => '',
        'section' => 'kleo_section_side_styling',
        'customizer' => true,
        'transport' => 'refresh'
    ...
    $kleo['set'][] = array(
        'id' => 'blog_full_bg',
        'type' => 'color',
        'title' => 'Full Style - Article media Background ',
        'default' => '',
        'section' => 'kleo_section_blog',
        'customizer' => true,
        'transport' => 'refresh',
        'description' => __('Background color for the Full Style Blog article page', 'kleo_framework'),
        'condition' => array( 'blog_style', 'full' )
    );

    $kleo['set'][] = array(
        'id' => 'blog_meta_elements',
        'type' => 'multi-select',
        'title' => __('Meta Fields', 'kleo_framework'),
        'description' => __('What meta fields do you want displayed?', 'kleo_framework'),
        'choices' => Kleo::get_config('blog_meta_elements'),
        'default' => Kleo::get_config('blog_meta_defaults'),
        'section' => 'kleo_section_blog',
        'customizer' => true,
        'transport' => 'refresh'
    );
...
add_action( 'wp_loaded', 'sq_check_side_bg', 12 );

function sq_check_side_bg() {
    // if side menu background is dark/white
    if ( sq_option( 'sidemenu_header_bg' ) ) {
        $sidemenu_color = sq_option( 'sidemenu_header_bg', false, true );
    } else {
        $sidemenu_color = sq_option( 'sidemenu_style_background_color', '#0c0e38', true );
    }
...
add_action( 'sq_options_after_save', 'kleo_delete_font_transient' );
...
function kleo_delete_font_transient() {
    ...
    delete_transient('kleo_font_icons');
}
...
/***************************************************
:: Blog full layout logic
 ***************************************************/

add_action('wp', 'sq_blog_full_actions');

function sq_blog_full_actions() {
	if ( is_singular( 'post' ) && sq_option( 'blog_style', 'standard', true ) == 'full') {
	    add_filter( 'kleo_page_title_class', 'sq_blog_page_title_class' );
	    add_filter( 'kleo_page_title_container_class', 'sq_blog_page_title_container_class' );
	    add_filter( 'kleo_entry_meta_classes', 'sq_entry_meta_class' );

	    add_action('kleo_page_title_section', 'sq_page_title_close_div', 17);
	    add_action('kleo_page_title_section', 'kleo_entry_meta', 18);
	    add_action('kleo_page_title_section', 'sq_page_title_media', 20);

	}
}


function sq_blog_page_title_class( $classes ) {
...
function sq_blog_page_title_container_class( $classes ) {

    $classes .= ' top-on-scroll fade-on-scroll';

    return $classes;
}

function sq_entry_meta_class( $classes ) {

    $classes .= ' fade-on-scroll-second bottom-on-scroll';
...
function sq_page_title_close_div() {
    echo '</div>';
}

function sq_page_title_media() {
        ...
        switch ( $kleo_post_format ) {
            ...
            default:
                if ( kleo_get_post_thumbnail_url() != '' ) {
                    ...
                    $image   = aq_resize( $img_url, 1200, null, true, true, true );
                    if ( ! $image ) {
                        $image = $img_url;
                    }
                    echo '<img class="image_fade image fade-on-scroll-half" src="' . $image . '" alt="' . get_the_title() . '">';
...
add_action( 'kleo_after_body', 'sq_add_side_trigger', 1);

function sq_add_side_trigger() {
    if( sq_option( 'header_status', true, true ) == false ) {
        ?>
