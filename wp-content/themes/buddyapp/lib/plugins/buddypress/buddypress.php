...
if ( ! function_exists( 'kleo_menu_user_avatar' ) ) {
    ...
    function kleo_menu_user_avatar( $item_output, $item, $depth, $args ) {
        ...
        if ( is_user_logged_in() ) {
            ...
            $output .= '<a title="' . bp_get_loggedin_user_fullname() . '" class="kleo-bp-user-avatar' . ( $args->has_children && in_array($depth, array(0,1)) ? ' open-submenu' : '' ) . '" href="' . $url . '" title="' . $attr_title .'">' . kleo_get_avatar() . $attr_title;
